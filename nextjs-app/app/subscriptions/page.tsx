...
import { CreditCard, TrendingDown, DollarSign, Calendar, AlertCircle, CheckCircle2, Wallet } from 'lucide-react';
...
export default function SubscriptionsPage() {
    ...
    const loadData = async () => {
        ...
        try {
            setLoading(true);
            const [subsData, unusedData] = await Promise.all([
                subscriptionsAPI.getSubscriptions(userId).catch(() => ({ subscriptions: [] })),
                subscriptionsAPI.getUnusedSubscriptions(userId).catch(() => ({ unused_subscriptions: [], total_potential_savings: 0 })),
            ]);
            setSubscriptions(subsData.subscriptions || []);
            setUnused(unusedData.unused_subscriptions || []);
            // Extract savings from unused subscriptions data
            setSavings(unusedData.total_potential_savings ? { potential_savings: unusedData.total_potential_savings } : null);
        } catch (error: any) {
            toast({
                title: 'Error',
